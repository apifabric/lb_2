...
resourcename="lb2"  # lower case, only
resourcegroup="lb2_rg"
...
# git clone https://github.com/apilogicserver/lb2.git
# cd lb2
...
az group create --name lb2_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group lb2_rg --sku S1 --is-linux
az container create --resource-group lb2_rg --name lb2 --image apilogicserver/lb2:latest --dns-name-label lb2 --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//lb2.westus.azurecontainer.io:5656'
